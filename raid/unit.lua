    ...
    Minimap.raid:SetScript('OnEnter', function()
        ...
        GameTooltip:SetText'点击显示/隐藏团队框架'
    ...
    f:SetScript('OnEvent', function()
        ...
        or (event == 'CHAT_MSG_SYSTEM' and string.find(arg1, '加入了一个团队。')) then
            local t = GetTime() + 60
            local y = -20
            local a = .5
            Minimap.raid.text:SetText'+'
        ...
        elseif event == 'CHAT_MSG_BG_SYSTEM_ALLIANCE' or event == 'CHAT_MSG_BG_SYSTEM_HORDE' then
            ...
            elseif string.find(s, 'The Alliance Flag was dropped') then
                ...
                unflag(t, index)
            elseif string.find(s, '- 联盟旗帜') then
                local t = gsub(s, '- 联盟旗帜 — (.+).', '%1')
                unflag(t, index)
            elseif string.find(s, '交还联盟旗帜!') then
                local t = gsub(s, '(.+) captured the Alliance flag!', '%1')
                unflag(t, index)
            elseif string.find(s, '夺取部落旗帜!') then
                local t = gsub(s, 'The Horde flag was dropped by (.+)!', '%1')
                unflag(t, index)
            elseif string.find(s, '- 部落旗帜') then
                local t = gsub(s, '- 部落旗帜 — (.+).', '%1')
                unflag(t, index)
            elseif string.find(s, '交还部落旗帜!') then
