...
---------------------------------------------------------------------------
	...
	local CastCraftPerform = function()
		local cast = '(.+)开始施放(.+)。' 						local fcast = string.find(arg1, cast)
		local craft = '(.+) -> (.+)。' 									local fcraft = string.find(arg1, craft)
		local perform = '(.+)开始施展(.+)。' 					local fperform = string.find(arg1, perform)
	...
	local HitsCrits = function()
		local hits   = '(.+)的(.+)击中(.+)造成(.+)点伤害。'      local fhits   = string.find(arg1, hits)
		local crits  = '(.+)的(.+)对(.+)造成(.+)点致命一击伤害。'     local fcrits  = string.find(arg1, crits)
		local absb   = '(.+)的(.+)被(.+)吸收了。'    local fabsb   = string.find(arg1, absb)

		local phits  = '你的(.+)击中(.+)造成(.+)点'         local fphits  = string.find(arg1, phits)
		local pcrits = '你的(.+)致命一击对(.+)造成(.+)点'        local fpcrits = string.find(arg1, pcrits)
		local pabsb  = '你吸收了(.+)的(.+)。'       local fpabsb  = string.find(arg1, pabsb)
	...
	local channelDot = function()
		local channelDot = '(.+)的(.+)使(.+)受到了(.+)点'		local fchannelDot = string.find(arg1, channelDot)
		local pchannelDot = '你受到(.+)点(.+)伤害（(.+)的(.+)）。'		local fpchannelDot = string.find(arg1, pchannelDot)

		local channelDotRes = '(.+)的(.+)被(.+)抵抗了。'		local fchannelDotRes = string.find(arg1, channelDotRes)
		local pchannelDotRes = '(.+)的(.+)被抵抗了。'				local fpchannelDotRes = string.find(arg1, pchannelDotRes)
	...
	local channelHeal = function()
		local hot  = '(.+)获得(.+)点生命值（(.+)的(.+)）。'      local fhot  = string.find(arg1, hot)
		local phot = '你因(.+)的(.+)而获得了(.+)点生命值。'		  local fphot = string.find(arg1, phot)
    ...
    local handleHeal = function()
        local h   = '你的(.+)治疗了(.+)(.+)点生命值。'
        local c   = '你的(.+)对(.+)产生极效治疗效果，恢复了(.+)点生命值。'
        local hot = '(.+)获得(.+)点生命值（你的(.+)）。'
