    ...
    f:SetScript('OnEvent', function()
        ...
        if n and string.find(n, '等级 (.+)') then
            if   string.find(n, '(玩家)') then
            ...
            if  c ~= 'normal' and c ~= 'minus' then
                local t = gsub(n, '等级 (.+) ((.+))', '%1')
                local classification = c == 'elite' and '(精英)' or c == 'rare' and '(稀有)' or c == 'rareelite' and '(稀有精英)' or '(Boss)'
                GameTooltipTextLeft2:SetText('等级 '..t..' '..classification)
            end
        end
        ...
        if  UnitExists'mouseovertarget' then
            ...
            GameTooltip:AddLine('|cfffec500目标:|r '..name, r, g, b)
        end
