	...
	function GameTimeFrame_UpdateTooltip(h, m)
		...
		if  tonumber(GetCVar'modClock') == 1 or TwentyFourHourTime then
			GameTooltip:AddDoubleLine(date'%H:%M', '|cffffffff当地时间|r')
			GameTooltip:AddDoubleLine(format(TEXT(TIME_TWENTYFOURHOURS), h, m), '|cffffffff服务器时间|r')
		else
			local pm = 0
			if h >= 12 then pm = 1 end
			if h >  12 then h = h - 12 end
			if h ==  0 then h = 12 end
			GameTooltip:AddDoubleLine(date'%I:%M %p', '|cffffffff当地时间|r')
			GameTooltip:AddDoubleLine(format(TEXT(pm == 0 and TIME_TWELVEHOURAM or TIME_TWELVEHOURPM), h, m), '|cffffffff服务器时间|r')
		end
		if  tonumber(GetCVar'modStopWatch') == 1 then
			GameTooltip:AddLine('点击切换秒表.', .3, 1, .6)
		end
		GameTooltip:AddLine('Shift + 点击切换24小时制.', .3, 1, .6)
		GameTooltip:Show()
	end
