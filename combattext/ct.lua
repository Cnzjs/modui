    ...
    local handler = function(event)
        ...
        elseif event == 'CHAT_MSG_SPELL_SELF_DAMAGE' then
            ...
            if cv == 1 then
                local h = '你的(.+)击中(.+)造成(.+)'  local hit  = string.find(arg1, h)
                local c = '你的(.+)致命一击对(.+)造成(.+)' local crit = string.find(arg1, c)
                if hit or crit then
                    local m = hit and h or crit and c
                    local t = gsub(arg1, '(.+)点(.+)伤害。', '%2')
                    if t then colour = SPELL_SCHOOL_COLORS[t] else colour = {r =  1, g =  1, b =  0} end
        			arg2 = gsub(arg1, m, '%3')  arg2 = gsub(arg2, '(.+)点(.+)伤害。', '%1')
        ...
        elseif event == 'CHAT_MSG_SPELL_SELF_BUFF' then
            ...
            if cv == 1 then
                local h    = '你的(.+)治疗了(.+)(.+)点生命值。'            local heal = string.find(arg1, h)
                local c    = '你的(.+)对(.+)产生极效治疗效果，恢复了(.+)点生命值。' local crit = string.find(arg1, c)
                local hot  = '(.+)获得(.+)点生命值（你的(.+)）。'
                if heal or crit then
                    arg2 = gsub(arg1, h, '%2%3')
                    colour = {r = .1, g = .7, b = .65}
                    if string.find(arg2, '(.+) — 你') then return end
